...
/** Client surface: invoices emitted for the pensioner's own payments. */
...
export class ClientInvoicesController {
  ...
  ): ReturnType<InvoicesService['listForClient']> {
    return this.invoices.listForClient(user.userId, query);
  }
  ...
  async pdf(
    ...
    const buffer = await this.invoices.pdfForClient(user.userId, id);
